...
import { usePostAccountApiAuthMutation, usePostAccountApiRegMutation } from "@/src/store/api/serverApiV4";
import Cookies from 'js-cookie';
import { setAlert } from "@/src/store/slices/alertationSlice";

type RegistrationState = 'registration' | 'authorization' | 'state3' | 'state4';
...
const AuthenticationModal: FC<AuthenticationModalProps> = ({open, handleClose}): ReactElement => {
    ...
    const [postAccountApiAuth] = usePostAccountApiAuthMutation()
    const [postAccountApiReg] = usePostAccountApiRegMutation()

    const handleOnClickAuth = async () => {
        if (authData.main_email.length && authData.password.length) 
            try {
                const result = await postAccountApiAuth(authData).unwrap()
                Cookies.set('token', result.token, { expires: 7 })
                dispatch(setHeaderInfo({
                    header_avatar_url: result.avatar,
                    header_fio: result.fio,
                    authorization: true
                }))
                handleClose()
            } catch (error: any) {
                if (error.status === 402) dispatch(setPassError(true))
                if (error.status !== 402) {
                    dispatch(setAlert({
                        message: 'Сервер авторизации временно недоступен',
                        severity: 'error',
                        open: true
                    }))
                }
    ...
    const handleOnClickReg = async () => {
        ...
        if (!none_email && !none_lastname && !none_name && !none_password && !!confirm_password.length) try {
            await postAccountApiReg(regData).unwrap()
            ...
            if (error.status === 424) dispatch(setExistEmailError(true))
            if (error.status !== 424) {
                dispatch(setAlert({
                    message: error.data?.detail ? error.data.detail as string : "Сервер регистрации временно недоступен",
                    severity: 'error',
                    open: true
                }))
            }
    ...
    const components: {
        ...
        { key: 'authorization', component: <AuthorizationComponent onKeyDown={handleOnClickAuth} />, title: 'Авторизация', buttonText: 'Войти', style: 'center' },
        // { key: 'passwordChange', component: PasswordChange, title: 'Смена пароля', buttonText: 'Сменить', style: 'center' },
    ];
                ...
                <Stack spacing={2}>
                    {currentComponent && (
                        <>
                            ...
                            <Button
                                type="submit"
                                onClick={len_error || confirm_error ? undefined : (registration === 'registration' ? handleOnClickReg : handleOnClickAuth)}
                                variant="contained"
