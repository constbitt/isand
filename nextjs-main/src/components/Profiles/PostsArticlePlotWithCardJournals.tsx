...
import ProfileRatingsCardJournals from "@/src/components/Profiles/ProfileRatingsCardJournals";
...
const PostsArticlePlotWithCard = () => {
    ...
    useEffect(() => {
        ...
        if ((selected_journals.length > 0 && selected_works.length > 0) || selected_laboratories.length > 0) {
            const req_authors = {
                selected_journals: selected_journals.map(item => item.id),
                ...
                path: thesaurus_path,
                selected_type: "journals",
                years: time_range
            } as PostsForGraphRequest;

            const req_labs = {
                selected_journals: selected_laboratories.map(item => item.div_id),
                selected_works_id: selected_works.map(item => item.id),
                level: level,
                selected_scheme_id: graph_type,
                cutoff_value: category_cutting,
        ...
        thesaurus_path, selected_journals, selected_works,
    ...
    return (
        selected_journals.length > 0 ? (
            <Stack direction={"row"} width={'80%'} alignSelf={'center'} spacing={2}>
                {loading ? <Loader /> :
                    <AnyBarChart data={selected_journals.length === 0 && selected_laboratories.length === 0 ? [] : chartData || []}
                                 onClick={(item) => {
                                 ...
                                 colors={(author_laboratory_id) => {
                                     const index = selected_journals.findIndex(a => a.id === author_laboratory_id);
                                     return generateColor(index);
                                 }}
                                 formatter={(author_laboratory_id) => {
                                     return selected_journals.find(a => a.id === author_laboratory_id)?.value || selected_laboratories.find(l => l.div_id === author_laboratory_id)?.div_name || "";
                                 }}
                                         ...
                                         return (
                                             <div style={{ background: "white", padding: "5px", borderRadius: "10px" }}>
                                                 ...
                                                 {Object.entries(payload[0].payload).filter(([key, value]) => {
                                                     ...
                                                     return <p
                                                         key={index} className="intro">{`${selected_journals.find(a => a.id === author_laboratory_id)?.value || selected_laboratories.find(l => l.div_id === author_laboratory_id)?.div_name || ""}: ${typeof value === "number" ? Math.round(value * 1000) / 1000 : value}`}</p>;
                                                 })}
                                             </div>
                                         );
                                     }
                    ...
                    />}
                {<Box>
                    <ProfileRatingsCardJournals curItem={curItem}/>
                </Box>}
            </Stack>
        ) : null
    );
