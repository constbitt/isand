...
const ProfileRatingsCardOrganizations = ({
    curItem
}: {
    ...
    useEffect(() => {
        if (curItem) {
            ...
            const a_req = {
                selected_organizations: tp ? selected_laboratories.map(item => item.div_id) : selected_organizations.map(item => item.id),
                ...
                <Stack style={{padding: "15px", height: "100%"}}>
                    {curItem ?
                        <Stack alignItems={"center"} height={"100%"} spacing={2}>
                            <Typography textAlign={"center"} fontWeight={"bold"}>{"Организация: "}{curItem.name}</Typography>
                            {
                                isPageLoading ?
                                    (<Loader/>)
                                    :
                        ...
                        : <Typography
                            sx={{color: "rgb(134, 142, 150)"}}>{"После клика на диаграмму здесь будет подробная информация об организации"}</Typography>
                    }
                </Stack>
            </Card>
        ) : null
...
export default ProfileRatingsCardOrganizations; 