...
const ProfileRatingsCard = ({
    ...
    useEffect(() => {
        if (curItem) {
            ...
            const a_req = {
                selected_conferences: tp ? selected_laboratories.map(item => item.div_id) : selected_conferences.map(item => item.id),
                ...
                path: path,
                selected_type: tp ? "labs" : "conferences",
                years: time_range
                ...
                <Stack style={{padding: "15px", height: "100%"}}>
                    {curItem ?
                        <Stack alignItems={"center"} height={"100%"} spacing={2}>
                            <Typography textAlign={"center"} fontWeight={"bold"}>{"Категория: "}{curItem.name}</Typography>
                            {
                                isPageLoading ?
                                    (<Loader/>)
                                    :
                        ...
                        : <Typography
                            sx={{color: "rgb(134, 142, 150)"}}>{"После клика на диаграмму здесь будет подробная информация о конференциях"}</Typography>
                    }
                </Stack>
            </Card>
...
export default ProfileRatingsCard;