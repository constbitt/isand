...
import ProfileRatingsCardCities from "@/src/components/Profiles/ProfileRatingsCardCities";
...
const PostsArticlePlotWithCardCities = () => {
    ...
    useEffect(() => {
        ...
        if ((selected_cities.length > 0 && selected_works.length > 0) || selected_laboratories.length > 0) {
            const req = {
                selected_cities: selected_cities.map(item => item.id),
        ...
        thesaurus_path, selected_cities, selected_works,
    ...
    return (
        selected_cities.length > 0 ? (
            <Stack direction={"row"} width={'80%'} alignSelf={'center'} spacing={2}>
                {loading ? <Loader /> :
                    <AnyBarChart data={selected_cities.length === 0 ? [] : chartData || []}
                                onClick={(item) => {
                                ...
                                colors={(author_id) => {
                                    const index = selected_cities.findIndex(a => a.id === author_id);
                                    return generateColor(index);
                                }}
                                formatter={(author_id) => {
                                    return selected_cities.find(a => a.id === author_id)?.value || "";
                                }}
                                        ...
                                        return (
                                            <div style={{ background: "white", padding: "5px", borderRadius: "10px" }}>
                                                ...
                                                {Object.entries(payload[0].payload).filter(([key, value]) => {
                                                    ...
                                                    return <p
                                                        key={index} className="intro">{`${selected_cities.find(a => a.id === author_id)?.value || ""}: ${typeof value === "number" ? Math.round(value * 1000) / 1000 : value}`}</p>;
                                                })}
                                            </div>
                                        );
                                    }
                    ...
                    />}
                {<Box>
                    <ProfileRatingsCardCities curItem={curItem}/>
                </Box>}
            </Stack>
        ) : null
    );
...
export default PostsArticlePlotWithCardCities; 